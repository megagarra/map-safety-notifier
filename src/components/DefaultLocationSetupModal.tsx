...
import { PinType, DefaultLocationInput } from '@/types';
...
interface DefaultLocationSetupModalProps {
  ...
  onSubmit: (data: DefaultLocationInput) => Promise<void>;
  existingDescription?: string;
  existingAddress?: string;
  existingType?: PinType;
}
...
export function DefaultLocationSetupModal({
  ...
  const [type, setType] = useState<PinType>(existingType ?? 'crime');
  const [description, setDescription] = useState(existingDescription ?? 'Ocorrências sem endereço confirmado');
  ...
  useEffect(() => {
    if (isOpen) {
      setType(existingType ?? 'crime');
      setDescription(existingDescription ?? 'Ocorrências sem endereço confirmado');
      setAddress(existingAddress ?? '');
      setError(null);
    }
  }, [isOpen, existingType, existingDescription, existingAddress]);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="w-[95vw] sm:max-w-[480px] bg-[#121212] border-[#2a2a2a] text-white">
        <DialogHeader>
          <DialogTitle className="text-white">Configurar marcador padrão</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="dl-addr" className="text-gray-300">Endereço / rótulo (opcional)</Label>
            <Input
              id="dl-addr"
              value={address}
              onChange={(e) => setAddress(e.target.value)}
              placeholder="Ex: Centro — local padrão"
        ...
        <DialogFooter className="gap-2">
          ...
          <Button onClick={handleSubmit} disabled={isSubmitting || !location} className="bg-violet-600 hover:bg-violet-700">
            {isSubmitting ? <Loader2 size={16} className="animate-spin" /> : 'Salvar marcador'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
